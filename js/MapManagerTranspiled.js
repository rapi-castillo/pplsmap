// Manually transpiled version of MapManager.js at https://babeljs.io/repl/
// For future use, we should have a way to automate this either locally or server-side
    ...
    var initialize = function initialize() {
      ...
      uniqueLocs.forEach(function (item) {
        ...
        if (item.className == 'group-meeting') {
          ...
          L.circleMarker(item.latLng, { radius: 4, className: item.className, color: 'white', fillColor: '#1462A2', opacity: 0.6, fillOpacity: 0.9, weight: 2 }).on('click', function (e) {
            _popupEvents(e);
          }).addTo(overlays);
        } else {
          L.circleMarker(item.latLng, { radius: 5, className: item.className, color: 'white', fillColor: '#a00003', opacity: 0.8, fillOpacity: 0.7, weight: 2 }).on('click', function (e) {
            _popupEvents(e);
          }).addTo(overlays);
        }
        // }, 10);
